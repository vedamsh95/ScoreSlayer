...
export function WizardCalculator({ player, round, initialBid = 0, initialWon = 0, initialMetadata, onUpdate }: WizardCalculatorProps) {
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      <View style={styles.displayRow}>
        ...
        <View style={styles.dividerBox}>
          <NeuTrench color="#150428" borderRadius={20} padding={10} style={styles.scoreSummary}>
            ...
            <Text style={[styles.summaryValue, { color: totalScore >= 0 ? "#00F5A0" : "#FF4757" }]}>
      ...
      <View style={styles.tipBox}>
        <NeuIconWell color="rgba(255,184,0,0.1)" size={40} borderRadius={12}>
          <MaterialCommunityIcons name="wizard-hat" size={24} color="#FFB800" />
        </NeuIconWell>
        <View style={styles.tipContent}>
          <Text style={styles.tipTitle}>WIZARD SCORING</Text>
          <Text style={styles.tipSub}>+20 for correct bid, +10 per trick. Wrong? -10 per trick off.</Text>
        </View>
      </View>
